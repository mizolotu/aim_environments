...
class PacketMonitor():
    ...
    def __init__(self, prcs_cbs, bridge_name='ids', time_threshold=0.1):
    ...
    def start(self):
        ...
        queue_td.setDaemon(True)
        queue_td.start()
        roll_td = Thread(target=self.process_packets)
        roll_td.setDaemon(True)
        roll_td.start()
    ...
    def process_packets(self):
        ...
        while True:
            ...
            # process the packets and/or payloads

            for item in self.cbs:
                cb = item['func']
                args = item['args']
                cb(headers, payloads, *args)
