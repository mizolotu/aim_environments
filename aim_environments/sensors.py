...
class SensorsEnv:
    ...
    def scan_attack(self):
        url, bee_ips, target_ips, potential_target_ips = self._attack(target_prefixes=['data', 'unknown'])
        dns_ips = []
        for container in self.containers['app']['device']:
            for dns_ip in container['dns']:
                if dns_ip not in dns_ips:
    ...
    def slowloris_attack(self):
        url, bee_ips, target_ips, potential_target_ips = self._attack(target_prefixes=['unknown'])
        dns_ips = []
        for container in self.containers['app']['device']:
            for dns_ip in container['dns']:
                if dns_ip not in dns_ips:
